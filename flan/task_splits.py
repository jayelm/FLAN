...
def full_task_name(
    abbrev_name: str, num_templates: int, shot_config: ShotConfig
) -> str:
...
def _get_default_task_collapse_map(
    num_templates: int, shot_config: ShotConfig
) -> Mapping[str, str]:
    ...
    for collapsed_task, tasks_to_collapse in _COLLAPSIBLE_TASKS_ABBREV.items():
        for task in tasks_to_collapse:
            ...
            collapsed_task_full_name = full_task_name(
                collapsed_task, num_templates, shot_config
            )
            collapse_map[task_full_name] = collapsed_task_full_name
    return collapse_map


def collapse_related_tasks(
    tasks: Sequence[str], collapse_map: Mapping[str, str]
) -> List[str]:
...
def expand_related_tasks(
    tasks: Set[str], expand_map: Mapping[str, Sequence[str]]
) -> Set[str]:
...
class TaskSplit:
    ...
    def __eq__(self, o) -> bool:
        return (
            self.name == o.name
            and self.train_tasks == o.train_tasks
            and self.test_tasks == o.test_tasks
        )
...
def generate_all_overlap_split(
    ...
    shot_config: ShotConfig = ShotConfig.ZERO,
) -> TaskSplit:
...
def generate_superglue_num_tasks_ablation(
    num_templates: int = 10, shot_config: ShotConfig = ShotConfig.ZERO
) -> List[TaskSplit]:
    ...
    for num_templates in flan_tasks.NUM_TEMPLATES_LIST:
        ...
        for num_train_clusters in _NUM_TRAIN_CLUSTERS_LIST:
            ...
            limited_task_split = TaskSplit(
                name=f"flan_diversity_split{num_train_clusters}"
                + f"_{num_templates}templates"
                + f"_{_LIMIT_NUM_TRAIN_TASKS}task_limit"
                + shot_config.name_suffix,
...
def generate_intra_cluster_splits(
    ...
    if not task_collapse_map:
        task_collapse_map = _get_default_task_collapse_map(num_templates, shot_config)
    ...
    for split_idx in range(_NUM_INTRA_CLUSTER_SPLITS):
        ...
        for cluster_idx, cluster_tasks in enumerate(task_clusters.values()):
            if len(cluster_tasks) <= 1:
                ...
                test_idx = select_holdout_task_idx(
                    split_idx, cluster_idx, len(cluster_tasks)
                )
...
def generate_inter_cluster_splits(
    ...
    task_splits = []
    for test_cluster_idx, (test_cluster_name, test_tasks) in enumerate(
        task_clusters.items()
    ):
        train_tasks = set()
        for cluster_name, cluster_tasks in task_clusters.items():
