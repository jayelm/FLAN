...
for dataset_name, cot_type, nlines in [
    ...
    COT_II_TASK_CONFIGS[
        f"{cot_type}_input_inversion_{dataset_name}"
    ] = COT_TASK_CONFIGS[f"{cot_type}_{dataset_name}"]
...
DIALOG_TASK_CONFIGS["wiki_dialog"] = TaskConfig(
    source=seqio.TfdsDataSource(tfds_name="wiki_dialog:1.0.0", splits=["train"]),
    preprocessors=[prep.wiki_dialog, prep.format_dialog],
    postprocess_fn=post.take_first_line,
    metric_fns=[t5_metrics.accuracy],
)
# '_input_inversion' will get mapped to an inverted template
DIALOG_II_TASK_CONFIGS["wiki_dialog" + "_input_inversion"] = DIALOG_TASK_CONFIGS[
    "wiki_dialog"
]
...
# '_input_inversion' will get mapped to an inverted template
DIALOG_II_TASK_CONFIGS["qrecc" + "_input_inversion"] = DIALOG_TASK_CONFIGS["qrecc"]
...
for task_name in constants_t0.T0_TRAIN_TASK_SPLITS:
    ...
    if "_score_eval" in subtask_id:
        ...
        constants_t0.T0_TRAIN_TASK_METADATA[task_name]["task_type"]
        == "t0_question_answer"
    ):
    ...
    T0_TASK_CONFIGS[task_name] = TaskConfig(
        source=seqio.TfdsDataSource(
            tfds_name=f"huggingface:bigscience__p3/{subtask_id}", splits=["train"]
        ),
...
NIV2_TASK_CONFIGS["tfds_natural_instructions"] = TaskConfig(
    source=seqio.TfdsDataSource(
        tfds_name="natural_instructions:1.0.1", tfds_data_dir=None, splits=["train"]
    ),
    preprocessors=[
        filter_mmlu_fn,
        lookup_posex_fn,
...
for t_name, config in ALL_CANDIDATE_TASK_CONFIGS.items():
    if prep.format_dialog in config.preprocessors or (
        prep.format_options in config.preprocessors
    ):
