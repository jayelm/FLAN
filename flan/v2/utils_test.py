...
def create_test_mixture(
    ...
    return seqio.MixtureRegistry.add(
        mixture_name, [(task1.name, task1_rate), (task2.name, task2_rate)]
    )
...
class UtilsTest(absltest.TestCase):
    ...
    def test_get_mixture_examples(self):
        ...
        mixture = create_test_mixture(
            dataset_size, length, task1_rate, task2_rate, task1_name, task2_name
        )
        ...
        examples, _ = utils._get_mixture_examples(
    ...
    def test_task_proportions(self):
        ...
        task_counts, _ = utils.compute_stats(
            mixture, num_steps, batch_size, task_feature_lengths
        )
        task1 = seqio.get_mixture_or_task(task1_name)
        task2 = seqio.get_mixture_or_task(task2_name)
        np.testing.assert_allclose(
            task_counts[task1] / task_counts[task2], task1_rate / task2_rate, rtol=1e-1
        )
