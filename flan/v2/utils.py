...
def inplace_modify_preprocessors(
    preprocessors: List[Callable[..., Any]],
    replacements=Mapping[Callable[..., Any], Callable[..., Any]],
):
...
class DecoderOnlyTaskIdFeatureConverter(seqio.PrefixLMFeatureConverter):
    ...
    MODEL_FEATURES = {
        ...
        "decoder_causal_attention": seqio.FeatureConverter.FeatureSpec(dtype=tf.int32),
        "task_ids": seqio.FeatureConverter.FeatureSpec(dtype=tf.int32),
    }
    ...
    def _convert_features(
        ...
        concat_length = task_feature_lengths["inputs"] + task_feature_lengths["targets"]
    ...
    def get_model_feature_lengths(
        ...
        decoder_length = (
            task_feature_lengths["inputs"] + task_feature_lengths["targets"]
        )
...
def _get_mixture_examples(mixture, num_steps, batch_size, task_feature_lengths):
    ...
    mixture_ds = mixture._sample_fn(
        datasets, rates, seed=42
    )  # pylint:disable=protected-access
    fc = DecoderOnlyTaskIdFeatureConverter(pack=True, use_custom_packing_ops=False)
...
def compute_stats(mixture, num_steps, batch_size, task_feature_lengths):
    ...
    examples, task_id_to_task = _get_mixture_examples(
        mixture, num_steps, batch_size, task_feature_lengths
    )
...
def reset_split_maxes_on_flan_v0_configs(
    ...
    for key, tconfig in original_flan_configs.items():
        ...
        if (
            (not isinstance(tconfig.source, seqio.dataset_providers.TfdsDataSource))
            or (
                key
                in [
                    "true_case",
            ...
            or (not tconfig.source._tfds_dataset._split_map)
        ):  # pylint: disable=protected-access
        ...
        reserve_exs = None
        old_split_map = (
            tconfig.source._tfds_dataset._split_map
        )  # pylint: disable=protected-access
        ...
        if val_neg_split != -1:
            ...
            reserve_exs = new_split_map["validation"][
                val_neg_split + 1 : end_reserve_val
            ]
...
def t_name_to_flan_pattern_name(t_name: str) -> str:
    ...
    if "para_crawl" in t_name:
        ...
        mapped_name = str(constants_t0.T0_TRAIN_TASK_METADATA[mod_t_name]["task_type"])
