...
def format_options(example, use_char_options_format: bool = False):
    ...
    if use_char_options_format:
        ...
        options = tf.reshape(
            tf.stack(
                [char_options[: len(example["options"])], example["options"]], axis=1
            ),
            [-1],
        )
    else:
...
def example_batch_to_list(example_batch, num_templates):
    ...
    Returns:
      A list of items.
    """
    ret = [{k: v[i] for k, v in example_batch.items()} for i in range(num_templates)]
...
def batch_apply_template(dataset, patterns_list):
    ...
    dataset_templated = dataset_batched.map(
        apply_template_to_batch, num_parallel_calls=tf.data.AUTOTUNE
    ).unbatch()
    ...
    return zipped_datasets.map(merge_features, num_parallel_calls=tf.data.AUTOTUNE)
...
def remove_unbatchable_items_ds(ds, training_keys):
    return ds.map(
        functools.partial(remove_unbatchable_items_ex, training_keys=training_keys)
    )
...
def rank_classification_from_options(
    ...
    def is_correct_fn(example):
        ...
        return tf.equal(
            example[options_key_name], tf.reshape(example[answer_key_name], [])
        )
...
def format_dialog_non_deterministic(example):
    ...
    dialog = tf.reshape(
        tf.stack(
            [item_names[: len(dialog)], dialog, opt_end_str[: len(dialog)]], axis=1
        ),
        [-1],
    )
...
@seqio.map_over_dataset
def boolq(
    example: Mapping[str, tf.Tensor], use_char_answer: bool = False
...
def arithmetic_addition(example):
    ...
    question, answer, num_digits = tf.io.decode_csv(
        example, record_defaults=[""] * 3, field_delim="\t", use_quote_delim=False
    )
    question = tf.strings.regex_replace(question, r"\\n", "\n")
    question = tf.strings.regex_replace(question, r"\n", "\n")
    output = dict(
...
def wiki_dialog(
    ...
    if random_turn:
        ...
        idx = (
            tf.random.uniform(shape=[], minval=0, maxval=maxval, dtype=tf.dtypes.int64)
            * 2
            + 1
        )
    else:
...
@seqio.map_over_dataset
def t0(
    example: tf.Tensor, multiple_choice: bool, use_char_answer: bool = False
) -> Mapping[str, tf.Tensor]:
    """Processes T0/P3.
...
@seqio.map_over_dataset
def capitalize(
    example: tf.Tensor, text_features: Iterable[str]
) -> Mapping[str, tf.Tensor]:
...
def add_constant_text_features(
    example: Dict[str, tf.Tensor], text_features: Mapping[str, Sequence[str]]
) -> Dict[str, tf.Tensor]:
