...
DEFAULT_MIXTURE_TASK_FILTERS = {
    "FLAN": None,
    "T0": lambda x: (constants_t0.T0_TASK_PREFIX not in x)
    or ("_score_eval" in x)
...
def register_mixture(
    ...
    mix_ex_cap: int,
    task_suffix: str,
    tasks_obj: typing.Union[typing.Dict[str, typing.Any], typing.List[str]],
...
def generate_mixture_suites(
    ...
    if not all([submix_key in DEFAULT_MIXTURE_TASKS for submix_key in submixtures]):
        raise ValueError(
    ...
    if not (0.0 <= ratio_answer_opts <= 1.0):
        raise ValueError("ratio_answer_opts needs to be between 0 and 1, inclusive.")
    ...
    for tsuffix in task_suffixes:
        ...
        automatic_mix_name = (
            f"{UNIVERSAL_MIX_PREFIX}_{task_str}"
            + f"_zs({ratio_zero_shot})_opts({ratio_answer_opts})"
            + tsuffix
        )
        final_mix_name = (
            f"{override_mix_name}{tsuffix}" if override_mix_name else automatic_mix_name
        )
