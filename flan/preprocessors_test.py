...
from flan import preprocessors, tasks, templates
...
class PreprocessorsTest(tf.test.TestCase, parameterized.TestCase):
    ...
    def test_remove_unbatchable_items_ds(self):
        ...
        training_keys = preprocessors.get_training_keys(templates.PATTERNS[task_name])
    ...
    def test_remove_trailing_spaces(self):
        example = {"foo": tf.constant("some trailing spaces    \n", dtype=tf.string)}
    ...
    def test_get_fewshot_num_tokens(self):
        ...
        output_features = {
            ...
            "targets": seqio.Feature(vocabulary=SimpleVocab(), add_eos=True),
        }
        rst = preprocessors.get_fewshot_num_tokens.__wrapped__(example, output_features)
